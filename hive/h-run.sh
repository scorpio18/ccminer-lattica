#!/usr/bin/env bash
# ccminer-lattica (Lattica LTA / sha3d) HiveOS runner.
...
# OpenSSL + jansson are linked statically into ccminer since 1.1, so no
# bundled libs are required; ./lib is kept on the path only for forward compat
# (harmless when absent). $(pwd) is the miner dir (we cd'd to it above).
