/*
 * Base58 / Bech32 address -> scriptPubKey for solo GBT coinbase.
 * Derived from pooler cpuminer (GPL-2.0).
 */
...
static bool b58dec(unsigned char *bin, size_t binsz, const char *b58)
{
	...
	int rem = (int)(binsz % 4);
	uint32_t remmask = 0xffffffffu << (8 * (unsigned)rem);
	...
	switch (rem) {
	case 3:
		*(bin++) = (unsigned char)((outi[0] >> 16) & 0xff);
	case 2:
		*(bin++) = (unsigned char)((outi[0] >> 8) & 0xff);
	case 1:
		*(bin++) = (unsigned char)(outi[0] & 0xff);
		++j;
	default:
		break;
	}
...
static bool bech32_decode(char *hrp, uint8_t *data, size_t *data_len, const char *input)
{
	...
	*(data_len) -= 6;
	for (i = 0; i < hrp_len; ++i) {
		int ch = (int)(unsigned char)input[i];
		if (ch < 33 || ch > 126)
			return false;
		if (ch >= 'a' && ch <= 'z')
			have_lower = 1;
	...
	for (i = 0; i < hrp_len; ++i)
		chk = bech32_polymod_step(chk) ^ (uint32_t)(input[i] & 0x1f);
	++i;
	while (i < input_len) {
		int v = (input[i] & 0x80) ? -1 : bech32_charset_rev[(int)(unsigned char)input[i]];
...
static size_t bech32_to_script(uint8_t *out, size_t outsz, const char *addr)
{
	...
	if (outsz < witprog_len + 2)
		return 0;
	out[0] = (uint8_t)(witver ? (0x50 + witver) : 0);
	out[1] = (uint8_t)witprog_len;
